...
// Dispatch a single command line that arrived from a source other than the USB
// CLI (e.g. an ASCII command frame on the Serial1 binary link). Runs the exact
// same parser as the USB CLI. The string is copied internally because the
// parser tokenizes in place, so the caller's buffer is left untouched.
