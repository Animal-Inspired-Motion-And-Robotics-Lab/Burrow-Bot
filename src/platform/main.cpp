...
//   [13..16] float   Rp (ohms)         smoothed (moving avg); rotated into the
//   [17..20] float   L  (uH)           calibrated frame when `rotated` is on (bit1)
...
void sendLDCResponse(const ldc1101_measurement_t& m, bool crackDetected,
  ...
  // Send the SMOOTHED sample so the `smoothing` window affects what the PC plots;
  // when rotation is on, send it rotated into the calibrated frame so the cloud
  // flattens. Both come from appendMeasurement(), which ran earlier this tick
  // (getLatestRotated* is the filtered sample after rotation).
  float rp = state.rotated ? getLatestRotatedRp() : getLatestFilteredRp();
  float l = state.rotated ? getLatestRotatedL() : getLatestFilteredL();
  memcpy(p + 13, &rp, 4);
  memcpy(p + 17, &l, 4);
